...
| 🗑 | コメントを全消去（確認あり） |
| 🚫 無視 | ノイズファイルを glob で非表示（入力欄。有効時は × で解除） |
| ⚙ 設定（右端） | 出力先 / 既定の比較先を設定 |
...
- 個別に **✎編集 / 🗑削除 / Resolve** が可能。**Resolve＝対応済み**。Resolve するとツリー・💬件数・ステータスバー・出力の全てから外れる（＝**ツリーに見えている＝出力される**で一貫）。エディタ上には残るので、そこから Unresolve できる。
- **行ズレ追従**: 再起動後、ファイルが変わっていてもアンカー行を頼りにコメント位置を追従。見つからない場合は警告ラベルが付く。
- **絞り込み（フィルタ）**: コメントのあるファイルだけに絞る。
- **クイックアクセス**: ステータスバーのコメント数をクリック、またはコマンド「コメントへ移動」で、全コメントを一覧 → 選んでジャンプ。

### 無視（ノイズ非表示）
- ツールバーの 🚫 から、**VS Code 検索の「除外するファイル」欄と同じ記法**で glob を入力すると、その差分をツリーから隠す。
- カンマ/改行区切りで複数指定。**スラッシュ無しは全階層のファイル名にマッチ**（例: `*.lock`、`Cargo.toml`）。`**`=任意階層 / `*`=`/`を跨がない / `?`=1文字。
- 例: `**/Cargo.toml, **/lib.rs, *.lock` で Rust の定型ノイズを一括非表示。
- 設定は workspaceState にローカル保存（再読込で残る・非コミット）。**入力を空にする or × で即解除**できるので「一時的」に使える。
- viewed や絞り込みと直交。**無視はツリーから隠すだけ**で、付いていたコメントや出力(`review.md`)には影響しない。
...
| `ponpokoReview.afterSubmit` | `clear` | Submit 後の出力コメントの扱い（`clear`/`resolve`/`none`） |
...
- Submit すると、**出力したコメントは既定で削除**（ツリーもエディタも消える。内容は `review.md` に残るので記録は失われない）。設定 `afterSubmit` で `clear`（既定）/ `resolve`（ツリーからは消すがエディタに残る）/ `none`（手動）を選べる。
